...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-webcam
  mpv av://v4l2:/dev/video0 \
  	--profile=low-latency --no-osc --untimed \
  	--demuxer-lavf-o=video_size=640x480,input_format=mjpeg \
  	--video-align-y=0.9 --video-zoom=0.8 \
  	--title=webcam "$@"
#+end_src
...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-bookmark
  ...
  notargs="-t 3000 -r 2597"

  if [[ -z "$site" ]]; then
      exit 0
  elif grep -qF "$site" "$file"; then
      notify-send ${notargs} "Oops.. $site" "Already in bookmark!"
      exit 0
  fi
  ...
  title="$(printf '%s\n' "${examples[@]}" | $W_MENU -l 2 -p "Title: " -mesg "=> ${site}" | sed 's/ /-/g')"

  # Add your org mode file path here (headings are in level 2 in my file)
  getheadings="$(rg --no-config "^\*\* " "${file}" | sed '/** gnus/q' | awk '{print $2 }')"
  # ^ prints heading upto "** elfeed"
  # i have added rest as rss feeds so

  tags="$(rg --no-config -o "__([[:alnum:][:nonascii:]_-]*)" "$file" | sed 's/__//g' | sort | uniq | $D_MENU -p "${title} =>  : " | sed 's/ /_/g')"
  ...
  # Sed appends the link in a clean way
      sed -i "/$section$/a + [[$site][${title}__${tags}]]" "$file" &&
          notify-send ${notargs} "Bookmark added!" " $site is now saved under ==>  $section"
  ...
  if grep -qF "$site" "$file"; then
      ...
      notify-send ${notargs} "Give a title & section to add mark"
  fi
...
** bibtex get bib reference
- To get bibliography reference fro the doi url
  #+begin_src shell :shebang "#!/usr/bin/env bash" :tangle bin/d-bibtex
...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-passentry
  ...
  password=$(cliphist list -max-items 10 | $D_MENU -p 'password: ' | cliphist decode)

  sleep "${sleepfor}" && wtype "${password}"
#+end_src
...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-todo
  ...
  cmd=$($D_MENU -l "$height" -p "$prompt" "$@" < "$file")
  while [ -n "$cmd" ]; do
...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-urls
  ...
  wtype "$(rg --no-config '^\+' ${file} | sed 's/+ //g' | ${L_MENU} -p 'Link: ' | rg --no-config -o '(http|https)://[a-zA-Z0-9./?=_%&:-]*')" 
...
#+begin_src sh  :shebang "#!/usr/bin/env bash" :tangle bin/d-clip
  source d-var.conf

  cliphist list | $L_MENU -p 'Clip: ' | cliphist decode | wl-copy
#+end_src

** Shell history
- Script to get shell command history from bash and fish shells via fuzzel

  #+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-hist
    source d-var.conf

    fish_hist="$HOME/.local/share/fish/fish_history"
    bash_hist="$HOME/.bash_history"
    tmp_hist="/tmp/histlist"

    sort ${fish_hist} | rg --no-config cmd | sed 's|- cmd: ||' | sort -u > $tmp_hist

    sort -u ${bash_hist} >> $tmp_hist

    cat $tmp_hist | rg --no-config '.{11,}' | $L_MENU | wl-copy

  #+end_src


** Script Variable conf
...
#+begin_src sh :tangle bin/d-var.conf
  ...
  # Options are : rofi, bemenu/dmenu, fuzzel
...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-ctrl
  ...
  case "$1" in
      ...
      "brightness")
          ...
          value="$(expr $(brightnessctl g) / 4)"

  esac

  notify-send "control value changed: $value%" -h int:value:$value -h string:synchronous:notify-music -r 2592 -u low -t 1000
#+end_src
...
if [ -z "$1" ]; then
	  wall1="$(find "$wall_dir" | shuf -n1)"
    wall2="$(find "$wall_dir" | shuf -n1)"
else
	wall1="$1"
  wall2="$2"
fi

cp "$wall1" ~/.local/share/bg1
cp "$wall2" ~/.local/share/bg2
...
#+begin_src sh :shebang "#!/usr/bin/env bash" :tangle bin/d-wifi
  ...
  notify-send "Getting list of available Wi-Fi networks..." -t 1000
  ...
  if [ "$chosen_network" = "" ]; then
  	...
  	if [[ $(echo "$saved_connections" | grep -w "$chosen_id") = "$chosen_id" ]]; then
  		nmcli connection up id "$chosen_id" | grep "successfully" && notify-send "Connection Established" "$success_message" -t 1000
  	else
  		if [[ "$chosen_network" =~ "" ]]; then
  			wifi_password=$(echo '' | $D_MENU -p "Password: " )
  		fi
  		nmcli device wifi connect "$chosen_id" password "$wifi_password" | grep "successfully" && notify-send "Connection Established" "$success_message" -t 1000
  	fi
  fi
...
#+begin_src bash :shebang "#!/usr/bin/env bash" :tangle bin/d-notify
  # audio=${1:-"/home/idlip/music/yt/misc/linux-notify.ogg"}
  audio=${1:-"/home/idlip/music/yt/misc/ltt-notify.ogg"}
...
#+begin_src bash :shebang "#!/usr/bin/env bash" :tangle bin/d-niri
  #shellcheck shell=bash
  windows="$(niri msg --json windows)"
  selection="$(echo "${windows}" \
    | jaq -r '.[] | "\(.app_id) - \(.title)\u0000icon\u001f\(.app_id)"' \
    | fuzzel --counter --dmenu --index --width 65 --lines "$(echo "$windows" | jaq length)"
  )"

  if [ "$selection" != -1 ]; then
    niri msg action focus-window --id "$(echo "${windows}" | jaq -c --arg INDEX "$selection" '.[$INDEX | tonumber].id')"
  fi
...
#+begin_src bash :shebang "#!/usr/bin/env bash" :tangle bin/d-type
...
** pastebin 0x0.st
#+begin_src bash :shebang "#!/usr/bin/env bash" :tangle bin/d-pastebin
  source d-var.conf
